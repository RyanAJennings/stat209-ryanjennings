...
```{r}
sum_by_type <- group_summarize(birds, type)

sum_by_type

ggplot (sum_by_type, aes(egg_mass_mean, male_mass_mean)) + 
  geom_point() + 
  scale_x_log10() + 
  scale_y_log10()
```
...
```{r}
five_bird_obsv <- filter(sum_by_type, n >= 5)
...
ggplot (sum_by_type, aes(egg_mass_mean, male_mass_mean)) + 
  ...
  scale_x_log10() + 
  scale_y_log10() +
  geom_point(color = "red", data = five_bird_obsv, size = 3) + 
  geom_text_repel( aes(x = egg_mass_mean, y = male_mass_mean, label = type), data = five_bird_obsv)
```
